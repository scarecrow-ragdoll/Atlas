...
<!--   Follow-up Coverage Beads - Records W03 coverage bead routing, closure evidence, and remaining final-gate work. -->
...
| WAVE-03 adds only `daily_logs`, `workout_exercises`, `workout_sets` for diary persistence; no `cardio_entries`, no `body_weight`, no `bodyWeight`. | `TestWorkoutMigrations_FilesExistWithGraceMarkup` checks prohibited migration tokens; `TestWorkoutMigrations_DailyLogSchema` checks no `body_weight`, no `bodyWeight`, and no `cardio_entries`; `REGRESSION_NO_SCOPE_PROOF.md` records product-only static no-scope proof with no matches. | COVERED / NON-GOAL | Closed by `Atlas-qb2.2.5`. |
| GraphQL/API exposes no cardio placeholders/enums/API fields and no body-weight fields. | `TestWorkoutGraphQLSchema_OperationSignaturesAndNoWave04Placeholders` checks the WAVE-03 operation signatures, Date scalar, and absence of `cardio_entries`, `CardioType`, `HeartRateZone`, `body_weight`, `bodyWeight`, `WorkoutDay`, and lower-case `cardio` placeholders in `apps/api/internal/atlas/graph/schema/workouts.graphql`; `REGRESSION_NO_SCOPE_PROOF.md` records the broader static audit and classifies the only broad hit as forbidden-token test data. | COVERED / NON-GOAL | Closed by `Atlas-qb2.2.5`. |
| Legacy `WorkoutDay` implementation names are not introduced. | `REGRESSION_NO_SCOPE_PROOF.md` records `rg -n -g '!**/*_test.go' "cardio_entries|CardioType|HeartRateZone|body_weight|bodyWeight|WorkoutDay" apps/api/internal/atlas apps/api/internal/repository/postgres/migrations apps/api/internal/repository/postgres/queries` with no product matches. | GATE / NON-GOAL | Closed by `Atlas-qb2.2.5`. |
| Broader non-goals stay absent: frontend/public web/web-admin routes or UI, charts/e1RM endpoint, AI export payload assembly, backup/import behavior, starter workout templates, and automatic working weight progression. | `REGRESSION_NO_SCOPE_PROOF.md` records `git diff --name-only origin/master...HEAD -- apps/web apps/web-admin` with no changed paths and targeted backend/API term grep with no matches. | GATE / NON-GOAL | Closed by `Atlas-qb2.2.5`; `origin/master` is the actual comparison base in this worktree. |
...
| DailyLog repository access is user-scoped and blocks cross-user aggregate reads. | `TestWorkoutRepo_DailyLog_UserScopedIsolation` creates two users on the same date, proves separate DailyLog IDs, and verifies another user cannot load the first user's aggregate. | COVERED for repository isolation. | None. |
...
| Wrong-user exercise cannot be attached. | `TestWorkoutRepo_AddWorkoutExercise_RejectsOtherUsersExercise` proves no row and no version change; `REGRESSION_NO_SCOPE_PROOF.md` records WAVE-02 Exercise Library service, resolver, and Docker-backed repository compatibility checks. | COVERED | None. |
