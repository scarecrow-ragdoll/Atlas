...
<!--   LAST_CHANGE: 1.0.1 - Recorded Atlas-qb2.2.3 service coverage closure for behavior, conflict, null, insert, remove, and reorder paths. -->
<!-- END_CHANGE_SUMMARY -->
...
- `Atlas-qb2.2.4`: expand GraphQL resolver/API/schema contract coverage for operation signatures, success mapping samples, no-create query behavior, date-range validation, null argument mapping, negative `expectedVersion` mapping, add-set parent not-found mapping, Date scalar generated binding, and no cardio/body placeholders in schema.
